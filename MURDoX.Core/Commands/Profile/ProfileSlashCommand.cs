...
namespace MURDoX.Core.Commands.Profile
{
    public class ProfileSlashCommand : ApplicationCommandModule
    {
        ...
        public async Task Profile(InteractionContext ctx, [Option("user", "username")] DiscordUser member)
        {
            ...
            var user = discordUsers.Where(x => x.Id == member.Id).FirstOrDefault();
            var dbUser = db.Users!.Where(x => x.Username == member.Username).FirstOrDefault();
